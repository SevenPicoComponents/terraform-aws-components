...
locals {
  ...
  # Handle Outpost case where we use the Outpost AZ instead of multiple AZs
  public_subnet_availability_zones = local.public_enabled ? (
    var.outpost_arn == null ? 
    flatten([for z in local.vpc_availability_zones : [for net in range(0, local.public_subnets_per_az_count) : z]]) :
    try([data.aws_outposts_outpost.default[0].availability_zone], [])
  ) : []
  
  private_subnet_availability_zones = local.private_enabled ? (
    var.outpost_arn == null ? 
    flatten([for z in local.vpc_availability_zones : [for net in range(0, local.private_subnets_per_az_count) : z]]) :
    try([data.aws_outposts_outpost.default[0].availability_zone], [])
  ) : []

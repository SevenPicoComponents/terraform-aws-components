...
resource "aws_s3_bucket_logging" "self" {
  ...
  target_prefix = var.access_log_bucket_prefix_override == null ? "${local.account_id}/${module.s3_log_storage_context.id}/" : (var.access_log_bucket_prefix_override != "" ? "${var.access_log_bucket_prefix_override}/" : "")
}
